...
void device_service_on_ble_evt(device_service_t *service, const ble_evt_t *p_ble_evt) {
    switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GATTS_EVT_WRITE: {
            ...
            if (p_ble_evt->evt.gatts_evt.params.write.handle == service->unlock_char_handles.value_handle) {
                //static device_transaction_t transaction;
				...
				if (!is_unlocked) {
					...
					device_unlock_char_notification(&device_service, 1);
					is_unlocked = 0;
				}
            }
        } break;
...
/*void device_listAccounts_char_init(device_service_t *service) {
    uint32_t err_code;

    static uint8_t data[64];
    NRF_LOG_INFO("Loading accounts...");
    device_address_t addresses[10];
    int count = device_listAddresses(addresses);
    size_t len = pack_accounts((char*)data, sizeof(data), addresses, count*device_ADDRESS_SIZE);
    static char str[128];
    a2hexstr(str, (uint8_t*)data, len);

    ble_uuid_t char_uuid;
    char_uuid.uuid = BLE_UUID_device_CHAR_LIST_ACCOUNTS;
...
void device_unlock_char_init(device_service_t *service) {
    uint32_t err_code;
    uint8_t value[128] = {0xDE,0xAD,0xBE,0xEF};

    ble_uuid_t char_uuid;
    char_uuid.uuid = BLE_UUID_DEVICE_CHAR_UNLOCK;
    ...
    err_code = sd_ble_gatts_characteristic_add(service->service_handle,
                                   &char_md,
                                   &attr_char_value,
                                   &service->unlock_char_handles);
    NRF_LOG_INFO("Characteristic handle: 0x%04x", service->unlock_char_handles.value_handle);
    APP_ERROR_CHECK(err_code);
}
...
void device_service_init(device_service_t *service) {
    ...
    service_uuid.uuid = BLE_UUID_DEVICE_SERVICE;
    ...
    //device_listAccounts_char_init(service);
    device_unlock_char_init(service);
}

/*void device_listAccounts_char_notification(device_service_t *service, int32_t *value) {
...
void device_unlock_char_notification(device_service_t *service, uint8_t unlocked) {
    if (service->conn_handle != BLE_CONN_HANDLE_INVALID) {
        ...
        hvx_params.handle = service->unlock_char_handles.value_handle;
        hvx_params.type   = BLE_GATT_HVX_NOTIFICATION;
        hvx_params.offset = 0;
        hvx_params.p_len  = &len;
        hvx_params.p_data = &unlocked;

		NRF_LOG_INFO("Send unlock notification");
        sd_ble_gatts_hvx(service->conn_handle, &hvx_params);
        sd_ble_gatts_hvx(service->conn_handle, &hvx_params); //weird workaround for BleGattCoroutines library
    }
}
